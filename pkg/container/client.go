...
import (
		...
		"github.com/X4Applegate/watchtower/pkg/registry"
		"github.com/X4Applegate/watchtower/pkg/registry/digest"
		t "github.com/X4Applegate/watchtower/pkg/types"
	)
...
func (client dockerClient) WarnOnHeadPullFailed(c t.Container) bool {
		...
		return registry.WarnOnAPIConsumption(c)
}
...
func (client dockerClient) IsContainerStale(c t.Container, params t.UpdateParams) (stale bool, latestImage t.ImageID, err error) {
		ctx := context.Background()

		if c.IsNoPull(params) {
					log.Debugf("Skipping image pull.")
				} else if err := client.PullImage(ctx, c); err != nil {
					return false, c.SafeImageID(), err
				}

		return client.HasNewImage(ctx, c)
}

func (client dockerClient) HasNewImage(ctx context.Context, c t.Container) (hasNew bool, latestImage t.ImageID, err error) {
		currentImageID := t.ImageID(c.ContainerInfo().ContainerJSONBase.Image)
...
func (client dockerClient) PullImage(ctx context.Context, c t.Container) error {
		...
		if match, err := digest.CompareDigest(c, opts.RegistryAuth); err != nil {
					headLevel := log.DebugLevel
					if client.WarnOnHeadPullFailed(c) {
