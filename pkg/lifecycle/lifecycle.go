...
func ExecutePreChecks(client container.Client, params types.UpdateParams) {
		...
		if err != nil {
					log.WithError(err).Error("Failed to list containers for pre-check lifecycle hooks")
					return
				}
...
func ExecutePostChecks(client container.Client, params types.UpdateParams) {
		containers, err := client.ListContainers(params.Filter)
		if err != nil {
					log.WithError(err).Error("Failed to list containers for post-check lifecycle hooks")
					return
				}
		for _, currentContainer := range containers {
